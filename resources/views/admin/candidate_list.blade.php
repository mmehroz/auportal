			...
			<div class="row align-items-center">
				<div class="col">
					...
					<ul class="breadcrumb">
						...
						<a class=" btn btn-success btn-md"   style="float:Left;margin:1%;color: #fff;margin-left: -15.3%;margin-top: 2.9%;" href="{{ url('/downloadExcel') }}" >Export Excel </a>
					</ul>
				</div>
				<div class="col-auto float-right ml-auto">
					<div class="view-icons" style="display:flex;">
						<!-- <a href="{{url('/candidate_list')}}" class="grid-view btn btn-link" title="Grid-view"><i class="fa fa-th"></i></a> -->
						<a href="{{url('/candidatelist')}}" class="list-view btn btn-link" style="background: #fff; color: #777; border: 1px solid #e3e3e3" title="List-view"><i class="fa fa-bars"></i></a>
				...
				<div class="col-sm-12">
					<ul class="nav nav-tabs nav-tabs-bottom">
						<li class="nav-item">
								...
								<a class="" href="{{url('/irexperience_list')}}" ><i class="fa fa-eye m-r-5"></i>Inexperience List</a>&nbsp&nbsp
								<a class="" href="{{url('/irrelevent_list')}}" ><i class="fa fa-eye m-r-5"></i>Irrelevant List</a>
						</li>
					</ul>
				</div>
			</div>
...
function getimageview($id){
		window.open('{{ URL::to("/preemployementpdf")}}/'+$id,'_blank');
		// window.location.replace('{{ URL::to("/getimageview")}}/'+$id);
	}
...
// }
		...
		$('document').ready(function(){
			...
			function showdata(){
				...
				$.get('{{ URL::to("/candidate_listdata")}}',function(data){
