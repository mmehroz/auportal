    ...
    <div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row align-items-center">
				...
				<input type="hidden" name="hdnalbumid" id="hdnalbumid" value="{{$data}}">
				<div class="col-auto float-right ml-auto" style="margin-top: -4%;">
				@if( session()->get("role") ==	 1)
					<a href="#" onclick="uploadgallerymodal({{$data}})" class="btn add-btn" data-toggle="" data-target="#add_employee"><i class="fa fa-plus"></i>Upload</a>
				@endif
				</div>
			</div>
		</div>
...
<div id ='modals'></div>
<script>
	function uploadgallerymodal($id){
		$.get('{{ URL::to("/uploadgallerymodal")}}/'+$id,function(data){
	...
	$('document').ready(function(){
			...
			function showdata(){
				// alert($('hdnalbumid').val());
				$id = $('input[name=hdnalbumid]').val();
				$("#loader").show();
				
				$.get('{{ URL::to("/gallerylistdata")}}/'+$id,function(data){
